    ...
    public void serverTick(TickEvent.ServerTickEvent event)
    {
        if(event.phase == TickEvent.Phase.START)
        {
            ...
            for(int i = activeTrades.size() - 1; i >= 0; i--)
            {
                ...
                {
                    TreeMap<String, Integer> trader1Hats = Hats.proxy.tickHandlerServer.getPlayerHatsList(ti.trader1.getName());

                    TreeMap<String, Integer> trader2Hats = Hats.proxy.tickHandlerServer.getPlayerHatsList(ti.trader2.getName());
...
//                }
    ...
    {
        TreeMap<String, Integer> playerHatsList = Hats.proxy.tickHandlerServer.playerHats.computeIfAbsent(player, k -> new TreeMap<String, Integer>());
